...
of J for all possible indexes. Deltas not diagonalized
...
def get_symdec(GF_ob_up,GF_ob_dn,delta1,delta2,E): 
    """
    Orbital decomposition (also works if delta is not diagonal)
    J_ob contains every combination of indexes - 4 for loops
    ...
    for x in range(GN[1]):
        for y in range(GN[2]):
            for ie in range(len(E)):
                J_int_ob[ie] = delta1[x, x]*GF_ob_dn[ie][x, y]*delta2[y, y]*GF_ob_up[ie][y, x]

            J_ob[x, y] = -np.imag(simps(J_int_ob, E)) / twopi 
    ...
    for ie in range(len(E)):
        ...
        J_int_ob_mix[ie] = delta1[0, 4]*GF_ob_dn[ie][4,0]*delta2[0, 4]*GF_ob_up[ie][4, 0] + \
                            ...
                            delta1[4, 0]*GF_ob_dn[ie][0,0]*delta2[0, 4]*GF_ob_up[ie][4, 4]
                ...
                if m !=l:
                    for ie in range(len(E)):
                        ...
                        J_int_ob_mix[ie] += delta1[k, k]*GF_ob_dn[ie][k,l]*delta2[l, m]*GF_ob_up[ie][m, k] + \
                                            delta1[l, m]*GF_ob_dn[ie][m,k]*delta2[k, k]*GF_ob_up[ie][k, l]



    
    J_ob_mix = -np.imag(simps(J_int_ob_mix, E)) / twopi
...
for Rn in range(np.shape(Rs)[0]):
    ...
    if symdec:
        ...
        J_ob, J_ob_mix = get_symdec(GF_ji_up, GF_ij_dn,delta1,delta2,E)
        J_ob_d = J_ob[4:,4:]
        print("J_ob = ", J_ob)
